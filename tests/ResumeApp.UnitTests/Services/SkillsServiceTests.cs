...
namespace ResumeApp.UnitTests.Services
{
    public class SkillsServiceTests
    {
        ...
        {
            new object[] { new SkillSqlEntity() },
            new object[] { new SkillMongoEntity() }
        };
        ...
        public async Task WhenGetAllSkill_ThenItemsReturnedAndProjectCalledOnce<TEntity>(TEntity _) where TEntity : class
        {
            //Arrange
            var repoMock = new Mock<IRepository<TEntity>>();
            repoMock
                .Setup(r => r.ProjectAsync(It.IsAny<Expression<Func<TEntity, SkillDto>>>()))
                .ReturnsAsync(new List<SkillDto> { new SkillDto() });

            var validatorMock = new Mock<IValidator<SkillDto>>();
            var service = new SkillsService<TEntity>(repoMock.Object, validatorMock.Object);
            ...
            repoMock.Verify(m => m.ProjectAsync(It.IsAny<Expression<Func<TEntity, SkillDto>>>()), Times.Once);
        }
        ...
        public async Task WhenGetItemById_ThenCertificateReturnedAndFindByIdCalledOnce<TEntity>(TEntity _) where TEntity : class
        {
            ...
            var repoMock = new Mock<IRepository<TEntity>>();
            repoMock
                .Setup(r => r.FindByIdAsync(It.IsAny<Guid>(), It.IsAny<Expression<Func<TEntity, SkillDto>>>()))
                .ReturnsAsync(new SkillDto { Id = id });
            ...
            //Assert
            Assert.NotNull(item);
            repoMock.Verify(m => m.FindByIdAsync(It.IsAny<Guid>(), It.IsAny<Expression<Func<TEntity, SkillDto>>>()), Times.Once);
        }
        ...
        {
            //Arrange
            var itemForUpdate = new SkillDto();
            var repoMock = new Mock<IRepository<TEntity>>();
            repoMock

...
namespace ResumeApp.UnitTests.Services
{
    public class ContactsServiceTests
    {
        ...
        {
            new object[] { new ContactSqlEntity() },
            new object[] { new ContactMongoEntity() }
        };
        ...
        public async Task WhenGetAllContact_ThenItemsReturnedAndProjectCalledOnce<TEntity>(TEntity _) where TEntity : class
        {
            //Arrange
            var repoMock = new Mock<IRepository<TEntity>>();
            repoMock
                .Setup(r => r.ProjectAsync(It.IsAny<Expression<Func<TEntity, ContactDto>>>()))
                .ReturnsAsync(new List<ContactDto> { new ContactDto() });

            var validatorMock = new Mock<IValidator<ContactDto>>();
            var service = new ContactsService<TEntity>(repoMock.Object, validatorMock.Object);
            ...
            repoMock.Verify(m => m.ProjectAsync(It.IsAny<Expression<Func<TEntity, ContactDto>>>()), Times.Once);
        }
        ...
        public async Task WhenGetItemById_ThenCertificateReturnedAndFindByIdCalledOnce<TEntity>(TEntity _) where TEntity : class
        {
            ...
            var repoMock = new Mock<IRepository<TEntity>>();
            repoMock
                .Setup(r => r.FindByIdAsync(It.IsAny<Guid>(), It.IsAny<Expression<Func<TEntity, ContactDto>>>()))
                .ReturnsAsync(new ContactDto { Id = id });
            ...
            //Assert
            Assert.NotNull(item);
            repoMock.Verify(m => m.FindByIdAsync(It.IsAny<Guid>(), It.IsAny<Expression<Func<TEntity, ContactDto>>>()), Times.Once);
        }
        ...
        {
            //Arrange
            var itemToCreate = new ContactDto();
            var repoMock = new Mock<IRepository<TEntity>>();
            repoMock
        ...
        {
            //Arrange
            var itemForUpdate = new ContactDto();
            var repoMock = new Mock<IRepository<TEntity>>();
            repoMock

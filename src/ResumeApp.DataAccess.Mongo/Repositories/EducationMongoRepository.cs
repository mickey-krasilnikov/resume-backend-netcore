...
{
	public class EducationMongoRepository : IRepository<EducationMongoEntity>
	{
		private readonly IMongoDbContext<EducationMongoEntity> _context;

		public EducationMongoRepository(IMongoDbContext<EducationMongoEntity> context)
		{
			_context = context;
		}

		public async Task<IReadOnlyList<TProjected>> FilterByAsync<TProjected>(
			Expression<Func<EducationMongoEntity, bool>> filterExpression,
			Expression<Func<EducationMongoEntity, TProjected>> projectionExpression)
		{
		...
		public async Task<EducationMongoEntity> InsertOneAsync(EducationMongoEntity entity)
		{
			await _context.Collection.InsertOneAsync(entity);
			return entity;
		}

		public async Task InsertManyAsync(ICollection<EducationMongoEntity> documents)
		{
			await _context.Collection.InsertManyAsync(documents);
		}

		public async Task ReplaceOneAsync(EducationMongoEntity entity)
		{
			await _context.Collection.FindOneAndReplaceAsync(_context.GetFilterById(entity.Id), entity);
		}

		public async Task DeleteOneAsync(Expression<Func<EducationMongoEntity, bool>> filterExpression)
		{
		...
		public async Task DeleteManyAsync(Expression<Func<EducationMongoEntity, bool>> filterExpression)
		{
